...
Adj_array=[]
final_f=[250]
for frame in final_f:
        Adj_array.append(element)
	for x in range(0,len(p)):
		for z in range(0,HYPER/p[x]):
			Job_Nodes+=1
	frame_count=HYPER/frame
	Adj_Matrix = [[0 for x in range(0,frame_count+Job_Nodes+2)] for x in range(0,frame_count+Job_Nodes+2)]

	for x in range(0,frame_count+Job_Nodes+2):
		Adj_Matrix[x][x]=0
	...
	for x in range(0,frame_count):
		...
		element+=1
		Adj_array.append(element)
		Adj_Matrix[element][frame_count+Job_Nodes+2-1]=frame
		for y in range(0,len(p)):
			for z in range(0,HYPER/p[y]):
				if(not((x*frame)<((z)*p[y]))and(not(((z*(p[y])+D[y])<((x+1)*frame))))):
	...
	Adj_array.append(element)
	print "*************** "
	MAX_FLOW_ADJ=max_flow(Adj_Matrix,0,frame_count+Job_Nodes+2-1,frame,Job_Nodes,frame_count)
...
fp_out.close()
array=[]
fp_main_out=open("main_output_1","w+")
